...
> **📹 CAD Joint Motion Video** > File: `Videos/freecompress-POSHA_fusion.mp4`  
> *Demonstrates the full kinematic chain: gear mesh → encoder magnet sweep → load cell arms deflecting. This recording shows the jig scrubbing through the full servo angular travel.* > *To record: Fusion 360 → File → New Animation → scrub timeline → Publish Video.*
...
*Reference Image:* `Images/bldc_based aproach.jpeg`
...
Two firmware variants are provided in the `Codes/` folder:
...
The DC motor torque control loop uses a **PI current controller** ($K_p = 8.0, K_i = 0.5$) with anti-windup. The outer sweep advances the torque setpoint by 0.05 N·m every 500 ms; the inner loop converges in under 200 ms.
...
Linearity is verified with a one-sample t-test ($\alpha = 0.05, n = 5, t_{crit} = 2.776$). Scale factors are stored in EEPROM.
...
|---|---|---|---|
| Position sensor wear | Potentiometer wiper erosion | Jitter / dead spots in output | Magnetic encoder (XL430) |
| Motor winding burnout | Sustained stall current | ↑ no-load current, ↓ stall torque | Stall watchdog (200 ms) |
| Gear backlash | Plastic tooth deformation | Hysteresis in oscillation (> 0.5°) | Metal gear upgrade |
| PWM noise corruption | Commutator switching EMI | Erratic position steps | Shielded cable + 100 nF cap |
| Voltage rail droop | Simultaneous servo actuation | Voltage dip at connector | 470–1000 µF bulk cap |
...
|---|---|---|
| **Temperature overshoot** | Pre-store thermal energy before liquid addition | Software only |
| **Pre-heat before confirm** | Begin heating as soon as recipe is selected | Recipe parser + UI prompt |
| **Overlap dispense + heat** | Dispense water/stock while heating | Recipe-specific flags |
| **Torque-based stirring** | Motor current = resistance proxy | Adaptive current threshold |
...
* Split-unit architecture — computation/power electronics shelf-mounted separately from the cooking module
...
* Compare against recipe nominal timing; adjust preheat and overshoot parameters
...
.
├── CAD_files/
│   ├── POSHA.f3z                  # Fusion 360 parametric assembly
│   └── QC_jig_static.step          # Static STEP export
├── Codes/
│   ├── posha_qc_module.ino        # Arduino Uno + breakout modules
│   └── posha_qc_pcb.ino           # ATmega328P + discrete ICs
├── Images/
│   ├── bldc_based aproach.jpeg     # FOC concept sketch
│   ├── bottom_left.png
│   ├── bottom_right.png
...
│   └── top_right.png
├── Videos/
│   └── freecompress-POSHA_fusion.mp4 # Fusion 360 kinematic demo
├── schematic/                     # Detailed hardware design files
├── FInal_submission.pdf           # Full written technical report
└── README.md
